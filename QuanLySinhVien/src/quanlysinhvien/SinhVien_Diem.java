...
//    SinhVien sv = lh.getSinhVien(taiKhoan.getTenTaiKhoan());
    ...
    private void addTableScore(String _idSubject) {
        ...
        int stt = 1;
        List<Diem> listDiem = DiemDAO.getListDiem(select,taiKhoan.getTenTaiKhoan());
        if (!listDiem.isEmpty()) {
    ...
    private void addComboBox(){
        List<Lop_MonHoc> listlop_MH = Lop_MonHocDAO.getListLop_MonHoc(taiKhoan.getTenTaiKhoan());
        DefaultComboBoxModel cbModel = new DefaultComboBoxModel();
        if(listlop_MH.size() > 0) {
            for(Lop_MonHoc item : listlop_MH){
                System.out.printf(item.getTenLopMH());
                cbModel.addElement(item.getTenLopMH());
            }
