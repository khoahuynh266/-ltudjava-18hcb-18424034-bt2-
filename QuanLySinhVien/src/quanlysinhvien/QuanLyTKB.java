...
public class QuanLyTKB extends javax.swing.JFrame {
    ...
    private void addDataForTableTKB() {
        ...
        System.out.printf("select: " + select);
        ArrayList<ThoiKhoaBieu> listThoiKhoaBieu = LopHocDAO.getListThoiKhoaBieu(select);
        if (listThoiKhoaBieu.size()> 0) {
            int stt = 1;
            DefaultTableModel tableModel = new DefaultTableModel();
            tableModel.setColumnIdentifiers(columName);
            for (ThoiKhoaBieu tkb : listThoiKhoaBieu) {
                String[] rows = new String[7];
                rows[0] = String.valueOf(stt);
                rows[1] = String.valueOf(tkb.getId());
                rows[2] = tkb.getMaMonHoc();
                rows[3] = tkb.getTenMonHoc();
                rows[4] = tkb.getPhongHoc();
                rows[5] = tkb.getHocKy();
                rows[6] = tkb.getNamHoc();

                tableModel.addRow(rows);
                stt++;
            }
            jtableTKB.setModel(tableModel);
        } else {
            DefaultTableModel tableModel = new DefaultTableModel();
            tableModel.setColumnIdentifiers(columName);
            jtableTKB.setModel(tableModel);
